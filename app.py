...
def json_score():
    try:
        ...
        score = clf.predict_proba(X)[0,0] * 1000
...
def web_score():
    try:
        ...
        # fill missing fields from application and convert types
        data['credit_amount'] = int(data['credit_amount'])
        data['credit_term'] = int(data['credit_term'])
        data['app_rank'] = int(data['app_rank'])
        data['app_count'] = int(data['app_count'])
        data['children16'] = int(data['children16'])
        ...
        if data['exp_all'] is None:
            ...
            data['exp_all'] = float(data['exp_all'])

        if data['exp_loans'] is None:
            data['exp_loans'] = 0
        else:
            data['exp_loans'] = float(data['exp_loans'])
    ...
    return render_template('score.html', response=response)
